...
class Clock(QWidget):
    ...
    def draw_clock(self, painter):
        ...
        for i in range(12):
            ...
            self.hour_stop_x = self.start_x + int((0.85 * self.clock_radius) * math.cos(self.hour_angle))
            self.hour_stop_y = self.start_y - int((0.85 * self.clock_radius) * math.sin(self.hour_angle))
            
            # painter.drawText(self.hour_stop_x - 7, self.hour_stop_y + 7, str(i))
            painter.drawText(self.hour_stop_x - 12, self.hour_stop_y - 12, 25, 18, Qt.AlignCenter, str(i))
    ...
    def update_minutehand(self, painter, start_x, start_y):
        ...
        self.minute_stop_x = start_x + int((0.7 * self.clock_radius) * math.cos(self.minute_angle))
        self.minute_stop_y = start_y - int((0.7 * self.clock_radius) * math.sin(self.minute_angle))
    ...
    def paintEvent(self, event):
        ...
        print(f"Current radius: {self.clock_radius}")
